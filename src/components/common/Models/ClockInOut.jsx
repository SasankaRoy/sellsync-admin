...
export const ClockInOut = ({
  ...
  const [punchInTime, setPunchInTime] = useState(null); // Store punch in time
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (response.data.workTime?.punch_in_time) {
          setPunchInTime(new Date(response.data.workTime.punch_in_time));
        }
      }
    } catch (error) {
      const errorMsg =
      ...
      // If already punched in, set status accordingly
      if (
        errorMsg &&
        (errorMsg.includes("already punched in") ||
          errorMsg.includes("already completed your work"))
      ) {
        setPunchStatus("punched-in");
        // Try to get the punch in time from the response if available
        if (error?.response?.data?.workTime?.punch_in_time) {
          setPunchInTime(new Date(error.response.data.workTime.punch_in_time));
        }
      ...
      >
        ...
        >
          ...
          <div className="my-5 flex flex-col gap-4">
            ...
            {punchStatus === "completed" && punchInTime && punchOutTime && (
