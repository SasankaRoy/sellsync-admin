...
import { CustomerAddEditModel } from "./CustomerAddEditModel";
...
export const ViewCustomerDetailsModel = ({ setFindCustomerModel }) => {
  ...
  return (
    <>
          ...
          <div className="my-3 flex flex-col gap-3">
            ...
            <div className="flex gap-5">
              ...
              <button
                ...
                className="bg-(--button-color1) flex justify-center items-center rounded-full text-white h-[3dvw] w-[3dvw] shrink-0 cursor-pointer"
              >
                <Search />
              </button>
            </div>
              ...
              <div className="w-full px-5 my-5  border-t border-gray-200 py-3">
                <div className="grid grid-cols-2 gap-5">
                  ...
                  <div className="flex flex-col gap-2">
                    <lable className="text-base sm:text-lg md:text-xl lg:text-[1dvw] font-normal paraFont">
                      Customer Points{" "}
                      {customerDetails?.offAmount > 0 && (
                        <span className="text-[1dvw] mainFont font-semibold text-green-500">
                    ...
                    <p className="text-[.8dvw] paraFont font-semibold text-gray-600">
                      * Redeem over {data?.minimum_points_required_for_redeem}{" "}
                      points.
                    </p>
                  </div>
                  <div className="flex flex-col gap-2">
                    ...
                    <input
                      className="bg-[#F3F3F3] w-full font-semibold font-[var(--paraFont)] placeholder:text-[#333333]/40 text-base sm:text-lg md:text-xl lg:text-[1.1dvw] border border-[#d4d4d4] active:outline transition-all duration-300 ease-linear active:outline-[var(--button-color1)] focus:outline focus:outline-[var(--button-color1)] rounded-xl py-1.5 px-3"
                      readOnly
                      value={moment(customerInfo?.date_of_birth).format(
                        "YYYY-MM-DD",
                      )}
                    />
                  </div>
                ...
                <div className="mt-5 flex justify-end gap-5 items-center">
                  ...
                  {customerInfo?.customer_points >=
                    (data?.minimum_points_required_for_redeem || 0) && (
                    <button
            ...
            <>
              <div className="flex justify-center items-center my-3 text-[1.2dvw]">
                <p className="mainFont text-center font-semibold text-gray-400">
                  <button onClick={() => setNewCustomer({state:true,userData:null,forState:"Add"})} className="bg-[var(--button-color1)] cursor-pointer text-white rounded-md font-semibold hover:opacity-80 transition-all duration-300 px-6 py-2">
                    Add New Customer
                  </button>
                </p>
              </div>
            ...
            <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm rounded-lg p-5">
              <div className="bg-white w-[90%] max-w-md p-6 rounded-xl shadow-2xl border border-gray-200">
                <div className="flex justify-between items-center mb-6">
                  <h4 className="text-xl font-bold mainFont text-gray-800">
                    Redeem Points
                  </h4>
                  <button
                ...
                <div className="space-y-4">
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex flex-col gap-3 max-h-[40vh] overflow-y-auto pr-1">
                      {isRedeemListLoading ? (
                        ...
                        redeemRateList.filter(
                          (r) =>
                            Number(r.point) <=
                            Number(customerInfo?.customer_points),
                        ).length > 0 ? (
                        redeemRateList
                          .filter(
                            (r) =>
                          ...
                          .map((rate, idx) => (
                            ...
                            >
                              <div className="flex justify-between items-center">
                                <span className="font-semibold mainFont">
                                  {rate.point} Points
                                </span>
                                <span className="font-bold mainFont text-green-600">
                                  Get $ {rate.point_cost} Off
                                </span>
                              </div>
                            </div>
                          ))
                      ) : (
                        <p className="text-sm text-gray-500 paraFont">
                          No applicable offers for your current points.
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  {selectedRate && (
                    <div className="bg-green-50 p-4 rounded-lg border border-green-100 flex justify-between items-center">
                      <span className="text-green-700 font-medium paraFont">
                        You get off:
                      </span>
      ...
      {
        newCustomer.state &&(
            <CustomerAddEditModel setEditUserModel={setNewCustomer} userData={null} forState={newCustomer.forState} />
        )
      }
    </>
  );
