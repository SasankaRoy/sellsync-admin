...
const NewLogin = () => {
    ...
    const handleScreenRender = (currentscreen) => {
        switch (currentSelection) {
            case "ADMIN":
                return <CommonScreen currentscreen={currentSelection} setCurrentSelection={setCurrentSelection} setLoginInfo={setLoginInfo} loginInfo={loginInfo} loginHandler={loginHandler} />
    ...
    return (
        <div className='h-screen w-screen overflow-hidden relative'>
            <div className='top-0 left-0 w-full h-full absolute'>
                <img src={Loginbg23} alt='background-image' className='w-full h-full object-cover object-center' />
            </div>
            ...
            <div className='relative w-full h-full flex justify-center items-center bg-transparent z-20'>
                <div className='fixed top-0 w-full left-0  py-4 flex justify-between items-center px-10'>
                    <div className='h-[5dvw] w-[20dvw] mx-auto'>
                        <img src={FullLogo} alt='sell-sync-logo' className='w-full h-full object-contain object-center' />
                    </div>
                    <div className='text-white px-3'>
                        <h2 className='text-[.9dvw] font-normal tracking-wider'>{currentTimeDate}</h2>
                    </div>
                </div>
    ...
    return (<>
        <div className='flex flex-col justify-center items-center gap-16'>
            <div className='flex justify-center items-center gap-50 mb-5'>
                <div onClick={() => handleSelecteDashboard('ADMIN')} className='flex justify-center items-center flex-col gap-8 cursor-pointer buttonOutterWrapper'>
                    <button className='bg-white/10 customShadow rounded-full text-white  p-5'>
                        <ShieldUser size={140} />
                    </button>
                    <h2 className='text-white text-[2.6dvw] font-bold tracking-wider uppercase'>Admin</h2>
                </div>
                <div onClick={() => handleSelecteDashboard('EMPLOYEE')} className='flex justify-center items-center flex-col gap-8 cursor-pointer buttonOutterWrapper'>
                    <button className='bg-white/10 customShadow rounded-full text-white  p-5'>
                        <CircleUserRound size={140} />
                    </button>
                    <h2 className='text-white text-[2.6dvw] font-bold tracking-wider uppercase'>Employee</h2>
                </div>
            </div>
        </div>
    </>)
...
const CommonScreen = ({ currentscreen, setCurrentSelection, setLoginInfo, loginInfo, loginHandler }) => {
                ...
                <div className='w-full flex gap-6 flex-col justify-center items-center'>
                    {
                        !showPassField && (
                            <div className='w-[70%]'>
                                <input value={loginInfo.userId} type='text' placeholder='user name...' className='bg-white w-full py-2 text-black px-5 text-center text-[1.5dvw] mainFont rounded-3xl outline-none fonr-semibold' />
                            </div>
                        )
                    }
                    {
                        showPassField && (
                            <div className='w-[70%]'>
                                <input type='password' value={loginInfo.password} placeholder='*****' className='bg-white w-full py-2 text-black px-5 text-center text-[1.5dvw] mainFont rounded-3xl outline-none fonr-semibold' />
                            </div>
                        )
                    }
                </div>

                <AlphabetKeyboard setShowPassFeild={setShowPassFeild} showPassFeild={showPassField} setCurrentSelection={setCurrentSelection} setLoginInfo={setLoginInfo} loginHandler={loginHandler} />
            </div>
        </>
    )
} 