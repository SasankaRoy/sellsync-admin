...
import { getAllAttendenceLog } from "../../../utils/apis/LeaveAndAttendence";
...
export const AttendenceLog = () => {
  ...
  const { data:rowData, isLoading, isError } = useQuery({
    queryKey: ["get_attendenc_logs"],
    queryFn: async () => {
      const res = await getAllAttendenceLog();
      return res.data;
    },
  });
  // console.log(data)
  ...
  const [colDefs] = useState([
    ...
    },
    {
      ...
      cellRenderer: (amount) => {
        return moment(amount?.value).format("LT") || null;
      },
      flex: 1,
    },
    {
...

    {
      ...
      cellRenderer: (amount) => {
        return amount?.value.toFixed(2) || 0;
      },
      flex: 1,
    },
    {
      ...
      headerName: "Date",
      cellRenderer: (amount) => {
        return moment(amount?.value).format("DD-MM-YYYY");
      },
      flex: 1,
    },
        ...
        {isLoading ? (
          ...
          <div className="pb-14 w-full px-4 sm:px-6 lg:px-0 h-[calc(100vh-5rem)]">
            ...
            <div className="flex flex-col items-center  px-2">
              <div className="w-[100%] flex-col flex gap-2 my-5 bg-[var(--primary-color)] rounded-md border border-[#d4d4d4] px-2.5 py-2 h-[80dvh]">
                <div className="flex justify-between items-center gap-3 px-2 mb-3">
                  <h3 className="text-2xl sm:text-3xl lg:text-[1.4dvw] font-semibold text-[var(--mainText-color)]">
                    Attendance Logs
                  </h3>
                </div>
            ...
            /> */}
              ...
              <div className="w-[100%] flex-col flex gap-2  bg-[var(--primary-color)] rounded-md border border-[#d4d4d4] px-2.5 py-2 h-[80dvh]">
                <div className="flex justify-between items-center gap-3 px-2 mb-3">
                  <h3 className="text-2xl sm:text-3xl lg:text-[1.4dvw] font-semibold text-[var(--mainText-color)]">
                    Leave Requests
                  </h3>
